...
if ($id && (!$existing || (int) ($existing['agent_id'] ?? 0) !== $agentId)) {
    header('Location: ' . url('agent-portal/properties.php'));
    exit;
}
...
if ($existing && $_SERVER['REQUEST_METHOD'] === 'POST' && csrf_check() && isset($_POST['img_action'])) {
    ...
    unset($p);
    save_data($data);
    header('Location: ' . url('agent-portal/property-edit.php?id=' . $id));
    exit;
}
...
$values = $existing ?? [
    ...
    'entry_date' => '', 'description' => '', 'images' => [], 'agent_id' => $agentId, 'featured' => false,
];
    ...
    if (!csrf_check()) {
        ...
        if (!$errors) {
            ...
            if ($existing) {
                ...
                save_data($data);
                header('Location: ' . url('agent-portal/property-edit.php?id=' . $newId));
            }
            exit;
        }
    }
...
$portalTitle = $existing ? 'עריכת נכס' : 'נכס חדש';
require __DIR__ . '/includes/agent-header.php';
?>

<div class="admin-main-head">
  <h1><?= $existing ? 'עריכת נכס' : 'נכס חדש' ?></h1>
  <a href="<?= e(url('agent-portal/properties.php')) ?>" class="btn btn-outline btn-sm">חזרה לרשימה</a>
</div>
