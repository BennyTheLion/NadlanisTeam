...
if ($filterAgent > 0) {
    $properties = array_values(array_filter($properties, fn($p) => (int) ($p['agent_id'] ?? 0) === $filterAgent));
}
if ($filterStatus !== '') {
...
<div class="admin-card">
  <?php if ($properties): ?>
    <div class="admin-table-wrap">
      <table class="admin-table">
        <thead>
          <tr><th></th><th>כותרת</th><th>עיר</th><th>מחיר</th><th>סוכן</th><th>סטטוס</th><th>מומלץ</th><th></th></tr>
        </thead>
        <tbody>
          <?php foreach ($properties as $p): ?>
            <tr>
              ...
              <td><?= e($agentsById[(int) ($p['agent_id'] ?? 0)] ?? '—') ?></td>
              <td>
                <?php
              ...
              <td>
                <div class="admin-row-actions">
                  <a href="<?= e(url('admin/property-edit.php?id=' . $p['id'])) ?>" class="btn btn-outline btn-sm">עריכה</a>
                  <form method="post">
                    <?= csrf_field() ?>
  ...
  <?php else: ?>
    <p class="empty-note">לא נמצאו נכסים תואמים. <a href="<?= e(url('admin/property-edit.php')) ?>">הוסיפו נכס ראשון</a>.</p>
  <?php endif; ?>
</div>
